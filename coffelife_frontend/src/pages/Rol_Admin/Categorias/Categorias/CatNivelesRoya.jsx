...
export default function CatNivelesRoya() {
  ...
  const handleCreate = async (e) => {
    ...
    try {
      await api.post('/cat_niveles_roya', {
      ...
      setSuccess('Nivel de roya creado correctamente.')
      cargarNiveles()
    } catch (err) {
      setError(err?.response?.data?.message || 'Error al crear nivel de roya.')
    } finally {
      setLoading(false)
    }
  }
...
function EditModal({ row, onClose, onSaved }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await api.put(`/cat_niveles_roya/${row.idNivel}`, {
        nombre: form.nombre.trim(),
        descripcion: form.descripcion.trim(),
      })
    ...
    <div className="cat-modal-overlay" onClick={handleOverlay}>
      <div className="cat-modal-box">
        <div className="cat-modal-header">
          <h3 className="cat-modal-title">Editar nivel de roya</h3>
