...
export default function CatEstadosCultivo() {
  ...
  const cargarEstados = async () => {
    ...
    try {
      ...
      setError(err?.response?.data?.message || 'Error al cargar estados de cultivo.')
    } finally {
      setFetching(false)
    }
  }
  ...
  const handleCreate = async (e) => {
    ...
    try {
      await api.post('/cat_estados_cultivo', {
      ...
      setSuccess('Estado de cultivo creado correctamente.')
      cargarEstados()
    } catch (err) {
      setError(err?.response?.data?.message || 'Error al crear estado de cultivo.')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleToggleActivo = async (item) => {
    ...
    if (!window.confirm(`¿${accion} este estado de cultivo?`)) return

    setError('')
    setSuccess('')

    try {
      await api.put(`/cat_estados_cultivo/${item.idEstado}`, { activo: nextState })
      setSuccess(`Estado de cultivo ${accion}do correctamente.`)
      cargarEstados()
    } catch (err) {
...
function EditModal({ row, onClose, onSaved }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await api.put(`/cat_estados_cultivo/${row.idEstado}`, {
        nombre: form.nombre.trim(),
        descripcion: form.descripcion.trim(),
      })
    ...
    <div className="cat-modal-overlay" onClick={handleOverlay}>
      <div className="cat-modal-box">
        <div className="cat-modal-header">
          <h3 className="cat-modal-title">Editar estado de cultivo</h3>
